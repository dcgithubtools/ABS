...
class disloss(nn.Module):
    def __init__(self, reduction='mean', lambda1=1, lambda2=1, epsilon=1e-8,
             pdf='gaussian', **_):
        super(disloss, self).__init__()
        self.pdf = pdf

    def forward(self, mu, std, tl):
        std = std * 0.1 + 1e-8
...
class ContrastLoss(nn.Module):
    def __init__(self, loss_weight, d_func, t_detach = False, is_one=False):
...
class ContrastiveLoss(nn.Module):
    def __init__(self, loss_weight = 1.0, t_detach = False, is_one=False):
    ...
    def forward(self, teacher, student, neg, blur_neg=None):
        ...
        return self.loss_weight * self.forward_func(teacher_vgg, student_vgg, neg_vgg, blur_neg_vgg)
